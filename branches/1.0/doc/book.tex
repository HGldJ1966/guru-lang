...
written with \texttt{Forall}.  So the statement would be written this
way in \guru:
...
a proof of the formula \texttt{F*[t]}, which is the formula you get if
you substitute the witness \texttt{t} for the $*$ in \texttt{F*}.  So
here, we will write:
...
\texttt{existsi}-proof.  We can just use \texttt{refl (le x x)} as
the proof \texttt{P} of \texttt{\{(le x x) = (le x x)\}} mentioned
above.

You can see the importance of the formula context in \texttt{existsi}-proofs
by considering this modification of the proof:
...
\noindent In English, this formula says, ``for all \texttt{x} of type
\texttt{nat}, there exists a \texttt{y} of type \texttt{nat} such that
\texttt{x} is less than or equal to \texttt{y}.''  This uses the
\texttt{le} function for less-than-or-equal-to on the unary natural
numbers.  That function is defined in \texttt{guru-lang/lib/nat.g}.

To prove this formula, we must just show how to find, for every
\texttt{nat} \texttt{x}, a \texttt{nat} \texttt{y} such that
\texttt{x} is less than or equal to y \texttt{y}.  Of course, for any
\texttt{x}, there are an infinite number of numbers that would serve
for such a \texttt{y}: all the numbers greater than or equal to
\texttt{x}.  We must just pick one of them to serve as the witness of
the existential quantification (i.e., the value that has the desired
property).  We will pick \texttt{x} as the witness, since there is a
theorem \texttt{x\_le\_x} defined in \texttt{guru-lang/lib/nat.g}
which proves:
...
\begin{verbatim}
Forall(x:nat). Exists(y:nat). { (lt x y) = tt}
\end{verbatim}
...
\noindent Let us walk through this line by line (the line numbers are
not valid \guru syntax).
...
corresponding to \texttt{lelt\_trans} five inputs:
\begin{itemize}
...
\texttt{(plus x y)} converges to \texttt{z}.
...
\begin{comment}
\noindent Sometimes while we are incrementally developing a proof, it is useful
to see exactly what formula some subproof proves.  There is a way to do that in
\guru.  You simply use a \texttt{show}-proof.  The syntax is:
...
classification, as if we had a classification error.  
\end{comment}
