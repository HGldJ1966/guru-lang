...
\ \ \ \ \ \ \ \ \ \ \ \ \begin{tikzpicture}
\draw (-2,0) 
   node[fill=purple,ellipse,minimum height=2cm,minimum width=1cm](f) {\ };
\draw[-latex',black,very thick] (-0.25,1.5) -- (-2,1); 
\draw[-latex',black,dashed] (-0.25,1) -- (-1.8,0.9) ; 
\draw (0.55,1.5) node {\texttt{x\,:\,A}} ;
\draw (1.7,1) node {\texttt{y\,:\,\#<owned x> A}} ;
