...
lists.  The idea is that for each size of cell, we will keep a list of
cells that have been previously freed, and which have that size.  Such
a list is called a \emph{free list}.  When it is time to allocate a
new cell for a use of a constructor, we will check the free list
associated with the size of that constructor's cells.  If there is a
cell available on the free list, we will remove that cell from the
free list, and use it, instead of allocating new memory.  
...
For cells consisting of just 1 word, like cells for \texttt{Z}, we are
forced to use this one word to point to the next cell in the free
list.  For cells consisting of more than one word, we can instead
abuse the second word.  This will allow us to avoid overwriting the
constructor tag in the first word.  
...
time that a delete operation will take, if we clear cell contents eagerly,
right when the cell is deleted.
...
$O(k)$, where $k$ is the number of words in the cell.  A final twist
with clearing cells lazily is that we need to know exactly which clear
function to call, at the point when we reallocate a cell.  At that
point, we know only the size of the cell, and its constructor tag
(guaranteed not to be overwritten by a free list pointer, if the cell
contains at least one word besides its first one -- and this is the
only situation in which we must call clear).  But we need also to know
its runtime datatype, to call the proper clear function.  The solution
to this problem is to store the integer corresponding to that datatype
in the refcount part of the first word of the cell, at the moment when
the cell is freed.

The emitted C code for managing free lists holding cells with one additional
word after the first one is:

\begin{verbatim}
void *free_1 = (void *)0;

void delete_1(void *_x) {
  void **x = (void **)_x;
  x[1] = free_1;
  free_1 = x;
}

#define delete_gnat_gS delete_1
\end{verbatim}

\noindent The first line declares the free list \texttt{free\_1} for
cells with one additional word, and initializes this to $0$, which in
C is often used to represent the empty list.  Then the \texttt{delete}
function uses the statement \texttt{x[1] = free\_1} to abuse word 1 of
the cell \texttt{x} to point to the current start of the free list.
The free list pointer is then modified to point to \texttt{x}, making
\texttt{x} the new head of the list.  Finally, we define the delete
function for \texttt{S} cells just to be this delete function.

The delete function for the \texttt{gnat} type is similar to the one
shown before, except now, as mentioned, we must store the integer tag
for the type, in the refcount area of the first word of the cell.
This is for the benefit of the global clear function (which takes in
only a single pointer to a cell of otherwise unknown type).  That
function will use the type tag and the constructor tag to dispatch the
cell to the proper clear function for that specific type of cell:

\begin{verbatim}
void delete_gnat(void *x) {
  *(int *)x = (gnat << 8) | ctor(x); // store the type here for benefit of clear()
  switch ctor(x) {
  case op_gZ: 
    delete_gnat_gZ(x);
    break;
...
\item Draw a boxes-and-pointers diagram of the free lists as they
exist immediately after the call to \texttt{(dec ulist s)} in
\texttt{test3a.w}.  A box is drawn for a cell, and a pointer is drawn
between cells if the first cell references the second.  Show any cells
reachable from other pointers (besides the free list pointers) from
the cells in the free lists.  Associate a unique number with your
boxes (e.g., their address in memory), that you can refer to in the
next part.
